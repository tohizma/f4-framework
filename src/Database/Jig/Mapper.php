...
class Mapper extends Cursor
{
    ...
    public function find($filter = null, array $options = null, $ttl = 0, $log = true)
    {
        ...
        if (!$fw->CACHE || !$ttl || !($cached = $cache->exists(
            ...
            if (isset($options['group'])) {
                ...
                foreach ($data as $i => &$row) {
                    ...
                    foreach ($cols as $col) {
                        if ($prev_i != $i && array_key_exists($col, $row) &&
                            ...
                            // reduce/modify
                            $drop = !isset($this->reduce[$col]) || call_user_func_array(
                                $this->reduce[$col][0],
                                [&$prev,&$row]
                            ) !== false;
                        } elseif (isset($this->reduce[$col])) {
                            $null = null;
                            // initial
                            call_user_func_array($this->reduce[$col][0], [&$row,&$null]);
                        }
                    }
                    if ($drop) {
                        unset($data[$i]);
                ...
                // finalize
                if ($this->reduce[$col][1]) {
                    foreach ($data as $i => &$row) {
                        $row = call_user_func($this->reduce[$col][1], $row);
                        if (!$row) {
                            unset($data[$i]);
                        }
                        unset($row);
    ...
    */
    ...
    {
        $this->reduce[$key] = [$handler,$finalize];
    }
